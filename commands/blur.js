module.exports = {
  name: 'blur',
  description: 'Blur an image by a factor of <n>.',
  ...
  async execute (client, message, args) {
    ...
    var working = await message.channel.send('Blurring `' + att + "` by a factor of " + Number(args[0]).toFixed(1))
    message.channel.startTyping();
    client.Jimp.read(att).then(async (image) => {
      image.blur(Number(Number(args[0]).toFixed(1)));
